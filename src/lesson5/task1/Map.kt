...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((i, j) in mapA) {
        if (phones[i] == null) {
            phones[i] = mutableListOf(j)
            continue
        }
        if (j !in phones[i]!!) phones[i]!!.add(j)
    }
    for ((i, j) in mapB) {
        if (phones[i] == null) {
            phones[i] = mutableListOf(j)
    ...
    return phones.map { it.key to it.value.joinToString() }.toMap()
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    var answer = Pair<String?, Double>(null, Double.MAX_VALUE)
    stuff.forEach {
        if (it.value.first == kind && it.value.second <= answer.second)
            answer = Pair(it.key, it.value.second)
    }
    return answer.first
}
